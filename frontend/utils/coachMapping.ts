...
// '2.5xR', '3.0xR', 'raise_2.5x', and 'jam'. The UI presents buttons for
// specific actions, which may include dynamic raise sizes based on
// presets (e.g. ['2x','2.5x','3x']). This function normalises the coach
// bucket into an action key used by the action bar. When the bucket
// specifies a raise size that is not present in the presets, the
// nearest available size is chosen. If nothing matches a reasonable
// fallback the function returns null to indicate that no highlight
// should be applied.

/**
 * Convert a coach bucket into a UI action key.
 *
 * @param bucket The bucket returned by the coach (e.g. 'fold', '2.5x', '2.5xR', 'raise_2.5x').
...
export function mapCoachToAction(
  ...
  // Direct preset-style buckets, including variants with a trailing "r"
  // (e.g. '2.5x' or '2.5xr' / '2.5xr' from labels like '2.5xR').
  ...
  // and tolerate a trailing "r" suffix (e.g. 'raise_2.5xr' / 'raise_2.5xr').
